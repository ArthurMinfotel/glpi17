...
//Get all groups from GLPI DB for the current entity and the subentities
                ...
                if ($oldgroups == 1) {
                    ...
                    if (!empty($infos)) {
                        ...
                        foreach ($iterator as $glpigroups) {
                            if (str_contains($glpigroups['ldap_group_dn'], $onlycn)
                                && !in_array(Toolbox::strtolower($glpigroups['ldap_group_dn']), $all_ldap_groups)) {
                                $group = new Group();
        $values['id'] = $glpigroups['id'];
                                ...
                                $values['is_notify'] = 0;
                                $values['is_manager'] = 0;
                                $values['is_itemgroup'] = 0;
                                $values['is_usergroup'] = 0;
                                $group->update($values);
                            }
                        }
                    }
...
/*
                    $iterator = $DB->request([
                        ...
                        'WHERE' => [
                                'is_assign' => 0,
                                'NOT' => ['ldap_group_dn' => NULL],
                                'sync_field_group' => NULL
                            ] + getEntitiesRestrictCriteria('glpi_groups', '', $entity)
                    ]);
